...
input_lables = tf.placeholder(tf.float32, [lab_dim, None])
# Set model weights
...
z = tf.nn.softmax(output, axis=0)

a1 = tf.argmax(tf.nn.softmax(output, axis=0), axis=0)  # 按行找出最大索引，生成数组
b1 = tf.argmax(input_lables, axis=0)
err = tf.count_nonzero(a1 - b1)  # 两个数组相减，不为0的就是错误个数

cross_entropy = tf.nn.softmax_cross_entropy_with_logits(labels=input_lables, logits=output, dim=0)
loss = tf.reduce_mean(cross_entropy, axis=0)  # 对交叉熵取均值很有必要
...
with tf.Session() as sess:
    ...
    for epoch in range(maxEpochs):
        ...
        for i in range(np.int32(len(Y) / minibatchSize)):
            ...
            _, lossval, outputval, errval = sess.run([train, loss, output, err], feed_dict={input_features: x1.T, input_lables:y1.T})
    ...
    y = -x * (sess.run(W)[0][0] / sess.run(W)[0][1]) - sess.run(b)[0][0] / sess.run(W)[0][1]
    plt.plot(x, y, label='first line', lw=3)

    y = -x * (sess.run(W)[1][0] / sess.run(W)[1][1]) - sess.run(b)[1][0] / sess.run(W)[1][1]
    plt.plot(x, y, label='second line', lw=2)

    y = -x * (sess.run(W)[2][0] / sess.run(W)[2][1]) - sess.run(b)[2][0] / sess.run(W)[2][1]
    ...
    for i in range(nb_of_xs):
        for j in range(nb_of_xs):
            ...
            o = sess.run(output, feed_dict={input_features: [[ xx[i, j]], [yy[i, j] ]]})
            classification_plane[i, j] = sess.run(a1, feed_dict={input_features: [[ xx[i, j]],
                                                                                    [yy[i, j] ]]})
